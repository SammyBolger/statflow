-- Silver: games (dbt version of src/statflow/transform/sql/games.sql).
--
-- Grain: one row per game. Extracts every field the model + dashboard need
-- from the raw schedule JSON payload. Deduplicates on game_pk keeping the
-- latest ingest — safe under re-ingest of the same date.
